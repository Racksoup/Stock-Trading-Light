...
router.put('/', async (req, res) => {
    try {
        await axios.put(`http://${process.env.HUE_BRIDGE_IP}/api/${process.env.HUE_USERNAME}/lights/1/state`, {on: true , hue: 29000, bri: 235});
...
router.get('/market', async (req, res) => {
    ...
    marketState = await axios.request(options).then((res) => {
...
router.put('/light', async (req, res) => {
    try {
        ...
        if (marketState === '"CLOSED"' ) {
            await axios.put(`http://${process.env.HUE_BRIDGE_IP}/api/${process.env.HUE_USERNAME}/lights/1/state`, {on: true, hue: 00000, bri: 85});
